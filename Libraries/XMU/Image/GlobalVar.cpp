...
float RateOut_Stand = 0.0f,
       ...
       SpeedOut = 0,
       radius = 0;              //转弯半径
float g_errorD = 0;//方向偏差
float g_angle_set = 0;//设置角度值
float g_speed_set = 0;//速度设置
int g_mode = 3;//模式
float g_drive_left = 0;//电机
float g_drive_right = 0;
float g_nLeftMotorPulseSigma = 0;//编码器
float g_nRightMotorPulseSigma = 0;
float g_duty_left = 0;//给定占空比
float g_duty_right = 0;
int deviation = 10;//转向偏差
int stop_flag = 0;//减速
